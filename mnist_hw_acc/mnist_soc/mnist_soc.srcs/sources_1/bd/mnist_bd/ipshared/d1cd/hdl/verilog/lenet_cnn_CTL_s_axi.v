...
localparam
    ...
    ADDR_PARAMETERS_DATA_0 = 6'h18,
    ADDR_PARAMETERS_CTRL   = 6'h1c,
    ADDR_FC6_OUTPUT_DATA_0 = 6'h20,
    ADDR_FC6_OUTPUT_CTRL   = 6'h24,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_parameters = 'b0;
    reg  [31:0]                   int_fc6_output = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_FC6_OUTPUT_DATA_0: begin
                    rdata <= int_fc6_output[31:0];
                end
            endcase
        end
    end
...
assign fc6_output = int_fc6_output;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_fc6_output[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_FC6_OUTPUT_DATA_0)
            int_fc6_output[31:0] <= (WDATA[31:0] & wmask) | (int_fc6_output[31:0] & ~wmask);
    end
end
