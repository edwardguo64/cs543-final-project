...
end entity lenet_cnn_CTL_s_axi;
...
architecture behave of lenet_cnn_CTL_s_axi is
    ...
    constant ADDR_PARAMETERS_DATA_0 : INTEGER := 16#18#;
    constant ADDR_PARAMETERS_CTRL   : INTEGER := 16#1c#;
    constant ADDR_FC6_OUTPUT_DATA_0 : INTEGER := 16#20#;
    constant ADDR_FC6_OUTPUT_CTRL   : INTEGER := 16#24#;
    ...
    signal int_parameters      : UNSIGNED(31 downto 0) := (others => '0');
    signal int_fc6_output      : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_FC6_OUTPUT_DATA_0 =>
                        rdata_data <= RESIZE(int_fc6_output(31 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    fc6_output           <= STD_LOGIC_VECTOR(int_fc6_output);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FC6_OUTPUT_DATA_0) then
                    int_fc6_output(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_fc6_output(31 downto 0));
                end if;
            end if;
        end if;
    end process;
